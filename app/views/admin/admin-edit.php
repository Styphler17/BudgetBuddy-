<div class="space-y-8">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-white/10 shadow-sm overflow-hidden max-w-2xl">
        <form id="admin-edit-form" action="" method="POST" class="p-8 space-y-6">
            <div class="space-y-2">
                <label for="admin_name" class="text-sm font-bold text-gray-700 dark:text-slate-300">Full Name</label>
                <input id="admin_name" type="text" name="name" value="<?php echo htmlspecialchars($admin['name'] ?? ''); ?>" class="w-full h-10 border border-gray-300 dark:border-white/10 bg-white dark:bg-slate-800 rounded-md px-3 text-sm focus:ring-2 focus:ring-primary/20 dark:focus:ring-accent/20 dark:text-white outline-none" required>
            </div>

            <div class="space-y-2">
                <label for="admin_email" class="text-sm font-bold text-gray-700 dark:text-slate-300">Email Address</label>
                <input id="admin_email" type="email" name="email" value="<?php echo htmlspecialchars($admin['email'] ?? ''); ?>" class="w-full h-10 border border-gray-300 dark:border-white/10 bg-white dark:bg-slate-800 rounded-md px-3 text-sm focus:ring-2 focus:ring-primary/20 dark:focus:ring-accent/20 dark:text-white outline-none" required>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    <label for="admin_role" class="text-sm font-bold text-gray-700 dark:text-slate-300">Role</label>
                    <select id="admin_role" name="role" class="w-full h-10 border border-gray-300 dark:border-white/10 rounded-md px-3 text-sm focus:ring-2 focus:ring-primary/20 dark:focus:ring-accent/20 dark:text-white outline-none bg-white dark:bg-slate-800">
                ...
                <div class="space-y-2">
                    <label for="admin-edit-pass" class="text-sm font-bold text-gray-700 dark:text-slate-300"><?php echo $mode === 'create' ? 'Initial Password' : 'New Password (leave blank to keep)'; ?></label>
                    <div class="relative">
